...
/**
 * This class validates that the defined enums are consistent with the DB data
 * that they represent at application startup.
 */
@Slf4j
...
public class EnumIntegrityValidator {
	...
	/**
	 * Validate that the provided enum entity is equals to one element of the DB
	 * list.
	 *
	...
	private <T> void checkIntegrity(List<T> dbList, Object enumEntity) {
		...
		if (first.isEmpty()) {
			throw new DataIntegrityViolationException("The following enum's integrity is not valid: " + enumEntity);
		}
	}
}
