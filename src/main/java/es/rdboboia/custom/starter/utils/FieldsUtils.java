...
/**
 * This utility class provides methods to work with class fields (updates and
 * null checks).
 */
@Slf4j
public class FieldsUtils {
	...
	public static <T> void updateIfRequired(T target, T source) {
		try {
			Map<String, Method> methodMap = Arrays.stream(target.getClass().getDeclaredMethods())
					.collect(Collectors.toUnmodifiableMap(Method::getName, method -> method));

			List<Field> fieldsToUpdate = Arrays.stream(target.getClass().getDeclaredFields())
